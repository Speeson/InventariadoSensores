...
# Métricas para imports
IMPORT_OPS = Counter("route_import_operations_total", "Import operations", ["operation", "status"])
IMPORT_OP_TIME = Histogram("route_import_operation_duration_seconds", "Import operation durations", ["operation"])
IMPORT_ROWS_PROCESSED = Counter("route_import_rows_processed_total", "Rows processed in imports", ["status"])
...
def import_events_csv(
    ...
    IMPORT_OPS.labels(operation="import_events_csv", status="started").inc()
    with IMPORT_OP_TIME.labels(operation="import_events_csv").time():
        try:
            ...
            for idx, raw_row in enumerate(reader, start=2):
                ...
                try:
                    ...
                    if product_issue:
                        ...
                        review_rows += 1
                        IMPORT_ROWS_PROCESSED.labels(status="review").inc()
                        reviews.append(
                    ...
                    if product is None and would_create:
                        if dry_run:
                            ok_rows += 1
                            IMPORT_ROWS_PROCESSED.labels(status="ok").inc()
                            continue
                        try:
                            product = product_repo.create_product(
                                db,
                            ...
                            error_rows += 1
                            IMPORT_ROWS_PROCESSED.labels(status="error").inc()
                            errors.append(
                                ImportErrorResponse(
            ...
            IMPORT_OPS.labels(operation="import_events_csv", status="success").inc()
            ...
            IMPORT_OPS.labels(operation="import_events_csv", status="error").inc()
            raise
...
def import_transfers_csv(
    ...
    IMPORT_OPS.labels(operation="import_transfers_csv", status="started").inc()
    with IMPORT_OP_TIME.labels(operation="import_transfers_csv").time():
        try:
            reader = _decode_csv(file)
            ...
            IMPORT_OPS.labels(operation="import_transfers_csv", status="success").inc()
            return ImportSummaryResponse(
                batch_id=batch.id,
                dry_run=dry_run,
                total_rows=total_rows,
            ...
            IMPORT_OPS.labels(operation="import_transfers_csv", status="error").inc()
            raise
