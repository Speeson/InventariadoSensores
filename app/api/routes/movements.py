...
# Métricas para movements
MOVEMENTS_OPS = Counter("route_movements_operations_total", "Movement route operations", ["operation", "status"])
MOVEMENTS_OP_TIME = Histogram("route_movements_operation_duration_seconds", "Movement operation durations", ["operation"])
...
def list_movements(
    ...
    MOVEMENTS_OPS.labels(operation="list_movements", status="started").inc()
    with MOVEMENTS_OP_TIME.labels(operation="list_movements").time():
        try:
            ...
            if cached is not None:
                MOVEMENTS_OPS.labels(operation="list_movements", status="success").inc()
                return cached
            ...
            MOVEMENTS_OPS.labels(operation="list_movements", status="error").inc()
            raise
...
def movement_in(
    ...
    MOVEMENTS_OPS.labels(operation="movement_in", status="started").inc()
    with MOVEMENTS_OP_TIME.labels(operation="movement_in").time():
        try:
            ...
            MOVEMENTS_OPS.labels(operation="movement_in", status="success").inc()
            return MovementWithStockResponse(stock=stock, movement=movement)
        except inventory_service.InventoryError as e:
            MOVEMENTS_OPS.labels(operation="movement_in", status="error").inc()
...
def movement_out(
    ...
    MOVEMENTS_OPS.labels(operation="movement_out", status="started").inc()
    with MOVEMENTS_OP_TIME.labels(operation="movement_out").time():
        try:
            ...
            cache_invalidate_prefix("reports:top-consumed")
            cache_invalidate_prefix("reports:turnover")
            MOVEMENTS_OPS.labels(operation="movement_out", status="success").inc()
            return MovementWithStockResponse(stock=stock, movement=movement)
        except inventory_service.InventoryError as e:
            MOVEMENTS_OPS.labels(operation="movement_out", status="error").inc()
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
def movement_adjust(
    ...
    MOVEMENTS_OPS.labels(operation="movement_adjust", status="started").inc()
    with MOVEMENTS_OP_TIME.labels(operation="movement_adjust").time():
        try:
            ...
            cache_invalidate_prefix("reports:top-consumed")
            cache_invalidate_prefix("reports:turnover")
            MOVEMENTS_OPS.labels(operation="movement_adjust", status="success").inc()
            return MovementWithStockResponse(stock=stock, movement=movement)
        except inventory_service.InventoryError as e:
            MOVEMENTS_OPS.labels(operation="movement_adjust", status="error").inc()
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
def movement_transfer(
    ...
    MOVEMENTS_OPS.labels(operation="movement_transfer", status="started").inc()
    with MOVEMENTS_OP_TIME.labels(operation="movement_transfer").time():
        try:
            ...
            cache_invalidate_prefix("reports:top-consumed")
            cache_invalidate_prefix("reports:turnover")
            MOVEMENTS_OPS.labels(operation="movement_transfer", status="success").inc()
            ...
            MOVEMENTS_OPS.labels(operation="movement_transfer", status="error").inc()
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))