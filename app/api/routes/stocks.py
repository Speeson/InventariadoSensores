...
# Métricas para stocks
STOCKS_OPS = Counter("route_stocks_operations_total", "Stock route operations", ["operation", "status"])
STOCKS_OP_TIME = Histogram("route_stocks_operation_duration_seconds", "Stock operation durations", ["operation"])
...
def list_stocks(
    ...
    STOCKS_OPS.labels(operation="list_stocks", status="started").inc()
    with STOCKS_OP_TIME.labels(operation="list_stocks").time():
        try:
            if order_by != "id":
            ...
            if cached is not None:
                STOCKS_OPS.labels(operation="list_stocks", status="success").inc()
                return cached
            ...
            STOCKS_OPS.labels(operation="list_stocks", status="error").inc()
            raise
...
def get_stock(stock_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    STOCKS_OPS.labels(operation="get_stock", status="started").inc()
    with STOCKS_OP_TIME.labels(operation="get_stock").time():
        try:
            cache_key = make_key("stocks:detail", user.id if user else None, {"id": stock_id})
            cached = cache_get(cache_key)
            if cached is not None:
                STOCKS_OPS.labels(operation="get_stock", status="success").inc()
                return cached
            ...
            STOCKS_OPS.labels(operation="get_stock", status="error").inc()
            raise
...
def create_stock(payload: StockCreate, db: Session = Depends(get_db)):
    STOCKS_OPS.labels(operation="create_stock", status="started").inc()
    with STOCKS_OP_TIME.labels(operation="create_stock").time():
        try:
            ...
            STOCKS_OPS.labels(operation="create_stock", status="success").inc()
            return stock
        except Exception as exc:
            STOCKS_OPS.labels(operation="create_stock", status="error").inc()
            raise
...
def update_stock(stock_id: int, payload: StockUpdate, db: Session = Depends(get_db)):
    STOCKS_OPS.labels(operation="update_stock", status="started").inc()
    with STOCKS_OP_TIME.labels(operation="update_stock").time():
        try:
            stock = stock_repo.get(db, stock_id)
            if not stock:
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Stock no encontrado")
            ...
            cache_invalidate_prefix("stocks:list")
            cache_invalidate_prefix("stocks:detail")
            STOCKS_OPS.labels(operation="update_stock", status="success").inc()
            return stock
        except Exception as exc:
            STOCKS_OPS.labels(operation="update_stock", status="error").inc()
            raise